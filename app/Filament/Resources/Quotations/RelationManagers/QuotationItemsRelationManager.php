...
class QuotationItemsRelationManager extends RelationManager
{
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->headerActions([
                CreateAction::make()
                    ->after(function (QuotationItem $record): void {
                        $record->loadMissing(['quotation', 'vatRate']);
                        app(QuotationService::class)->recalculateItemTotals($record);
                        app(QuotationService::class)->recalculateDocumentTotals($record->quotation);
                    }),
            ])
            ->recordActions([
                EditAction::make()
                ...
                DeleteAction::make()
                    ->after(function (QuotationItem $record): void {
                        ...
                        if ($quotation) {
                            app(QuotationService::class)->recalculateDocumentTotals($quotation);
                        }
                    }),
            ])
            ->toolbarActions([

...
class CustomerDebitNoteItem extends Model
{
    ...
    protected static function booted(): void
    {
        static::updating(function (self $item): void {
            $parentStatus = $item->customerDebitNote?->status;
            ...
            throw new RuntimeException(
                'Cannot modify economic inputs of items on a non-Draft debit note '
                ."(#{$item->customerDebitNote?->debit_note_number}, status={$parentStatus->value}). "
        ...
        static::deleting(function (self $item): void {
            ...
            if ($parentStatus instanceof DocumentStatus && $parentStatus !== DocumentStatus::Draft) {
                throw new RuntimeException(
                    'Cannot delete items of a non-Draft debit note '
                    ."(#{$item->customerDebitNote?->debit_note_number}, status={$parentStatus->value})."
                );
            }
        });
    ...
    public function customerDebitNote(): BelongsTo
    {
        return $this->belongsTo(CustomerDebitNote::class);
    }

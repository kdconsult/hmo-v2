...
class SupplierCreditNote extends Model
{
    ...
    {
        $this->subtotal = $this->items->sum(fn (SupplierCreditNoteItem $item) => (float) $item->line_total);
        $this->tax_amount = $this->items->sum(fn (SupplierCreditNoteItem $item) => (float) $item->vat_amount);
